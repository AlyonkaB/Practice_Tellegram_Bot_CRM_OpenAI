...
async def hello(update: Update):
    ...
    await update.message.reply_text(
        f"Hello, I'v got your message." f" \\ {message_text}"
    )


async def begin(update: Update, context):
    await update.message.reply_text(
        "Choose one button", reply_markup=main_menu_keyboard()
    )
...
async def button_handler(update: Update, context):
    ...
    if query.data == "get_leads":
        await query.edit_message_text(
            text="List leads:", reply_markup=InlineKeyboardMarkup(list_leads_keyboard())
        )
        ...
        query.data in list_lead_id and query.message.text == "Select a lead to delete:"
    ):
        delete_lead(query.data)

    elif query.data == "back":
        await query.edit_message_text(
            "Main menu or previous options here.", reply_markup=main_menu_keyboard()
        )
